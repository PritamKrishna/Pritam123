rm(list=ls())
setwd("C:/Users/91888/OneDrive/Desktop/DataScience Project")
...
#Fare amount has a negative value, which doesn't make sense. A price amount cannot be -ve and also cannot be 0. So we will remove these fields.
...
#Checking 20 observations of passenger_count are consistenly above from 6,7,8,9,10 passenger_count
...
#Removing 58 observations and 20 observation which are above 6 value because a cab cannot hold these number of passengers.
train = train[-which(train$passenger_count < 1 ),]
train = train[-which(train$passenger_count > 6),]

#Latitudes range from -90 to 90.Longitudes range from -180 to 180.Removing those which do not satisfy these ranges
...
#There's only one outlier which is in variable pickup_latitude.So we will remove it with nan.Also we will see if there are any values equal to 0.
...
#There are values which are equal to 0. We need to remove them.
...
# Mode Method
getmode(train$passenger_count)  # We can't use mode method because data will be more biased towards passenger_count=1
...
#mae=3.188024      mse=16.325802      rmse=4.040520      mape=0.424659 
...
# mae=1.8981592       mse=6.7034713      rmse=2.5891063      mape=0.2241461
...
# mae = 1.9053850       mse = 6.3682283     rmse = 2.5235349      mape = 0.2335395
...
#mae = 1.6193378 mse = 5.2082001 rmse = 2.2821481 mape = 0.1836901
